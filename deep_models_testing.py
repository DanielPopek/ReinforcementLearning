...
def single_deep_learning_run(deep_learning_player_type='DeepQLearning', train_count=3000,
                             is_deep_player_cross=True, epochs=100, data_shape=9, filter=False, optimizer='adam',
                             loss_type='mean_squared_error', hidden_one_size=100, hidden_two_size=64, hidden_layers=1,
                             verbose=True):
    TEST_COUNT = 1000
    ...
    if deep_learning_player_type == 'NN':
        player = NNPlayer(board, CROSS if is_deep_player_cross else OH, train_count, epochs,
                          data_in=data_shape, filter=filter, optimizer=optimizer, loss_type=loss_type,
                          hidden_one_size=hidden_one_size, hidden_two_size=hidden_two_size, hidden_layers=hidden_layers)
    player.train_model()
...
def loss_impact_test(deep_learning_player_type, verbose=True):
    LOSS_TYPES = ['mean_squared_error', 'mean_absolute_error', 'mean_absolute_percentage_error',
        ...
        for input_shape in INPUT_SHAPES:
            for i in range(iterations):
                ...
                wins = single_deep_learning_run(deep_learning_player_type=deep_learning_player_type,
                                                ...
                                                epochs=5, data_shape=input_shape, filter=filtering, optimizer='adam',
                                                loss_type=loss, hidden_size=100, hidden_layers=1, verbose=verbose)
                df = df.append({'i': i, 'train_count': 5000, 'epochs': 10,
                                'x_wins': wins[2], 'o_wins': wins[0], 'draws': wins[1], 'loss': loss,
                                'in_shape': input_shape}, ignore_index=True)
...
def optimizer_impact_test(deep_learning_player_type, verbose=True):
    ...
    # need to change ending of file_name by hand
    file_name = f'{deep_learning_player_type}_in{in_shape}_iters{iterations}_isx{str(is_deep_player_x)}_' \
        f'lossAdam{"_FILTERING" if filtering else ""}_epochs{epochs[0]}_TC5000_OPTIMIZER'

    df = pd.DataFrame(columns=['i', 'train_count', 'epochs', 'x_wins', 'o_wins', 'draws', 'optim', 'in_shape'])

    for optim in OPTIMIZERS:
        for i in range(iterations):
            print(f'{i+1}. OPTIMIZER {optim}')
            wins = single_deep_learning_run(deep_learning_player_type=deep_learning_player_type,
                                            is_deep_player_cross=is_deep_player_x, train_count=5000,
                                            epochs=10, data_shape=in_shape, filter=filtering, optimizer=optim,
                                            loss_type='mean_squared_error', hidden_one_size=100, hidden_layers=1,
                                            verbose=verbose)
            df = df.append({'i': i, 'train_count': 5000, 'epochs': 10,
                            'x_wins': wins[2], 'o_wins': wins[0], 'draws': wins[1], 'optim': optim,
                            'in_shape': in_shape}, ignore_index=True)
...
def hidden_size_impact_test(deep_learning_player_type, verbose=True):
    ...
    for size in HIDDEN_SIZES:
        for i in range(iterations):
            ...
            wins = single_deep_learning_run(deep_learning_player_type=deep_learning_player_type,
                                            is_deep_player_cross=is_deep_player_x, train_count=5000,
                                            epochs=10, data_shape=in_shape, filter=filtering, optimizer='adam',
                                            loss_type='mean_squared_error', hidden_size=size, hidden_layers=1,
                                            verbose=verbose)
            df = df.append({'i': i, 'train_count': 5000, 'epochs': 10,
                            'x_wins': wins[2], 'o_wins': wins[0], 'draws': wins[1], 'hiden_size': size,
                            'in_shape': in_shape}, ignore_index=True)
...
def hidden_size_impact_test_with_2_layers(deep_learning_player_type, verbose=True):
    HIDDEN_SIZES = [10, 20, 35, 50, 75, 100, 150, 200, 300, 500]
    HIDDEN_INNER_SIZES = [10, 20, 50, 100, 200, 500]
    ...
    # need to change ending of file_name by hand
    file_name = f'{deep_learning_player_type}_in{in_shape}_iters{iterations}_isx{str(is_deep_player_x)}_' \
        f'lossAdam_optMSE{"_FILTERING" if filtering else ""}_epochs{epochs[0]}_TC{train_count}_TWO_HIDDEN_LAYERS'

    df = pd.DataFrame(columns=['i', 'train_count', 'epochs', 'x_wins', 'o_wins', 'draws', 'hidden_one_size', 'hidden_two_size', 'in_shape'])

    for layer_one in HIDDEN_SIZES:
        for layer_two in HIDDEN_INNER_SIZES:
            for i in range(iterations):
                print(f'{i+1}. HIDDEN LAYER SIZES: {layer_one}x{layer_two}')
                wins = single_deep_learning_run(deep_learning_player_type=deep_learning_player_type,
                                                is_deep_player_cross=is_deep_player_x, train_count=train_count,
                                                epochs=10, data_shape=in_shape, filter=filtering, optimizer='adam',
                                                loss_type='mean_squared_error',
                                                hidden_one_size=layer_one, hidden_two_size=layer_two, hidden_layers=2, verbose=verbose)
                df = df.append({'i': i, 'train_count': 5000, 'epochs': 10,
                                'x_wins': wins[2], 'o_wins': wins[0], 'draws': wins[1], 'hidden_one_size': layer_one,
                                'hidden_two_size': layer_two, 'in_shape': in_shape}, ignore_index=True)

        df.to_csv('./csv_files/' + file_name + f'_ONLY_TO_{layer_one}_HS.csv')
    df.to_csv('./csv_files/' + file_name + '.csv')
    return file_name
    ...
    for input_shape in INPUT_SHAPES:
        for filter in FILTER:
            for i in range(iterations):
                ...
                wins = single_deep_learning_run(deep_learning_player_type=deep_learning_player_type,
                                                is_deep_player_cross=is_deep_player_x, train_count=5000,
                                                epochs=5, data_shape=input_shape, filter=filter, optimizer='adam',
                                                loss_type='mean_squared_error', hidden_size=100, hidden_layers=1,
                                                verbose=verbose)
                df = df.append({'i': i, 'train_count': 5000, 'epochs': 10,
                                'x_wins': wins[2], 'o_wins': wins[0], 'draws': wins[1], 'filter': filter,
                                'in_shape': input_shape}, ignore_index=True)
